## Hytale GUI Development

This section provides detailed information on developing Graphical User Interfaces (GUIs) for Hytale mods.
...
**Multiple HUDs:**
*   Show multiple custom HUDs at the same time: https://www.curseforge.com/hytale/mods/multiplehud
*   Example MultipleHUD code: https://github.com/underscore95/Hytale-Sandbox-Plugin/tree/multiple-huds

### .ui files
Hytale currently uses `.ui` files to render UI. These are currently deprecated, and Hytale is planning to move to NoesisGUI. The transition has not happened yet, and `.ui` files are currently the only way to render UI.
...
*   A `TextField` is an input where the user can enter text.
...
### HUDs
A HUD is an element of UI that stays on the screen all the time, for example, the player's hot bar or health bar. It cannot be interacted with.
...
### UI Pages
Pages are another type of UI; these prevent the player from interacting with the game and unlock the player's mouse. Some examples of pages include: Crafting menu, pause menu.
...
In many cases, you want to update your UI at runtime.
...
*   **Failed to apply Custom UI HUD commands**
    This means your `.ui` file has something wrong with it. Enabling Diagnostic Mode in Hytale settings might give more information.
